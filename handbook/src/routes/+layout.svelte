...
<svelte:window
	onclick={(e) => {
		if (sidebarOpen && !e.target.closest('#sidebar, #menu-toggle')) closeSidebar();
	}}
/>
...
<div id="layout">
	<nav id="sidebar" class={sidebarOpen ? 'open' : ''} aria-label="Handbook navigation">
		<div class="sidebar-inner">
			{#each navItems as item, idx (item.id)}
				{#if item.group}
					<span class="sidebar-label" style={idx > 0 ? 'margin-top:14px;display:block' : ''}
						>{item.group}</span
					>
				{/if}
				<a
