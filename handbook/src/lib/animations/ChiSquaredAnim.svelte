<script>
	...
	const DT = 1,
		NPT = 240,
		TRUE_P = 24; // 10 days, 1 h bins
	const SERIES = Array.from(
		{ length: NPT },
		(_, i) => 50 + 22 * Math.cos((2 * Math.PI * i * DT) / TRUE_P) + gauss() * 9
	);
	...
	const SWEEP = PERIODS.map((p) => {
		const f = fold(p);
		return { p, Qp: f.Qp, crit: chiCrit(f.df) };
	});
	...
	const A_ACC = isTest ? 0.55 : 0.34,
		A_CMP = isTest ? 1.0 : 0.5;
	let frame = 0,
		playing = false,
		raf = null,
		lastTime = null,
	...
	function resize() {
		if (cA) setupCanvas(cA);
		if (cB) setupCanvas(cB);
		draw(frame);
	}
	...
	function label(t) {
		if (isTest) {
			...
			return t >= A_ACC
				? `χ² = ${X2.toFixed(2)}, df = ${DF1}, p = ${(1 - chiCDF(X2, DF1)).toFixed(4)}`
	...
	function drawBars(t) {
		...
		CATS.forEach((c, i) => {
			...
			ctx.fillRect(x, Y(OBS[i]), w, T + ph - Y(OBS[i]));
			ctx.globalAlpha = 1;
			...
			if (i < shown) {
				...
				ctx.lineWidth = 2.4;
				ctx.beginPath();
				ctx.moveTo(x + w / 2, Y(OBS[i]));
				ctx.lineTo(x + w / 2, Y(EXP[i]));
				ctx.stroke();
		...
		ctx.setLineDash([5, 3]);
		ctx.beginPath();
		ctx.moveTo(L, Y(EXP[0]));
		ctx.lineTo(L + pw, Y(EXP[0]));
		ctx.stroke();
		ctx.setLineDash([]);
		ctx.fillStyle = C.red;
		ctx.font = '10px ' + F;
	...
	function drawFold(t) {
		...
		ctx.textAlign = 'center';
		ctx.fillText(
			`folded at ${trial.toFixed(1)} h  —  ${cycles} cycles overlaid`,
			L + pw / 2,
			T - 9
		);
		ctx.fillStyle = C.muted;
	...
	function drawRight(t) {
		...
		if (!sweeping) {
			...
			const stat = isTest
				? CONTRIB.slice(0, clamp(Math.floor((t / A_ACC) * CATS.length), 0, CATS.length)).reduce(
						(a, b) => a + b,
						0
					)
				: t >= A_ACC
			...
			if (stat > 0) {
				ctx.fillStyle = C.gold + '55';
				ctx.beginPath();
				ctx.moveTo(X(stat), T + ph);
				for (let x = stat; x <= xmax; x += xmax / 400) ctx.lineTo(X(x), Y(chiPDF(x, df)));
				ctx.lineTo(X(xmax), T + ph);
				ctx.closePath();
				ctx.fill();
			}
			ctx.strokeStyle = C.navy;
			...
			if (stat > 0) {
				...
				ctx.beginPath();
				ctx.moveTo(X(stat), T);
				ctx.lineTo(X(stat), T + ph);
				ctx.stroke();
				ctx.fillStyle = C.red;
				ctx.font = 'bold 10px ' + F;
				ctx.textAlign = X(stat) > L + pw * 0.7 ? 'right' : 'left';
				ctx.fillText(
					`statistic = ${stat.toFixed(2)}`,
					X(stat) + (X(stat) > L + pw * 0.7 ? -5 : 5),
					T + 12
				);
			}
			ctx.strokeStyle = C.border;
		...
		ctx.textAlign = 'right';
		ctx.fillText(
			'p = 0.05 threshold (rises with df)',
			L + pw - 4,
			Y(SWEEP[SWEEP.length - 1].crit) - 6
		);
		// Qp
		ctx.strokeStyle = C.navy;
		ctx.lineWidth = 2.4;
		...
		ctx.fillStyle = C.gold;
		ctx.beginPath();
		ctx.arc(X(SWEEP[upto].p), Y(SWEEP[upto].Qp), 4.5, 0, 2 * Math.PI);
		ctx.fill();
		ctx.strokeStyle = C.border;
		ctx.lineWidth = 1;
		ctx.strokeRect(L, T, pw, ph);
...
<div class="anim-embed" style={`height: ${H}`}>
	...
	<div class="viz-row one-col">
		<div class="chart-panel">
			<h3>
				{isTest
					? 'The χ² distribution, and the p-value as area'
					: 'χ² reference, then the periodogram it builds'}
			</h3>
	...
	<div class="legend">
		...
		{:else}
			<div class="legend-item">
				<div class="legend-swatch" style="background: var(--axis)"></div>
				Individual cycles
			</div>
			<div class="legend-item">
				<div class="legend-swatch" style="background: var(--navy)"></div>
