<script>
	...
	function drawTest(k) {
		...
		for (let x = 0; x <= pW; x++) {
			const t = (x / pW) * N;
			const y = padT + pH / 2 - Math.sin((2 * Math.PI * k * t) / N + phase) * pH * 0.42;
			x === 0 ? ctx.moveTo(padL + x, y) : ctx.lineTo(padL + x, y);
		}
		ctx.stroke();
		const periodH = (N / k).toFixed(1);
		const label = +periodH >= 2 ? `Period: ${periodH} h  (k = ${k})` : `k = ${k}`;
...
<div class="anim-embed" style={height ? `height: ${height}` : undefined}>
	...
	<div class="controls">
		<button class="btn" bind:this={btnPlay} onclick={fftToggle}>&#9654; Play</button>
