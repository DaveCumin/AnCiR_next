<script>
	...
	const sig = (() => {
		...
		return Array.from({ length: N }, (_, i) => {
			const v =
				0.5 + 0.4 * Math.sin(((2 * Math.PI) / 23.5) * i - Math.PI / 2) + (rng() - 0.5) * 0.12;
	...
	for (let lag = 0; lag <= LAG_MAX; lag++) {
		let sum = 0;
		for (let i = 0; i < N - lag; i++) sum += (sig[i] - mean) * (sig[i + lag] - mean);
	...
	function acfSeek(e) {
		...
		frame = Math.max(
			0,
			Math.round(Math.max(0, Math.min(1, (e.clientX - rect.left) / rect.width)) * TOTAL)
		);
		drawAll(frame);
	}
...
<div class="anim-embed" style={height ? `height: ${height}` : undefined}>
	...
	<div class="controls">
		<button class="btn" bind:this={btnPlay} onclick={acfToggle}>&#9654; Play</button>
