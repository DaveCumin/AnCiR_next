<script>
	...
	function draw(f) {
		...
		if (stepInd) {
			...
			stepInd.textContent =
				f >= TOTAL ? 'Scalogram complete' : `Probing period ${curPeriod.toFixed(1)} h`;
		}
	}
	...
	function drawSignal(period, probeT, done) {
		...
		ctx.lineWidth = 1.4;
		ctx.beginPath();
		ctx.moveTo(X(SWITCH), T);
		ctx.lineTo(X(SWITCH), T + ph);
		ctx.stroke();
		ctx.setLineDash([]);
		ctx.fillStyle = C.red;
		ctx.font = '10px ' + F;
	...
	function drawScalogram(rowsDone, within, curRow, done) {
		...
		const W = cScal.offsetWidth,
			H = cScal.offsetHeight;
		if (!W || !H) return;
		ctx.clearRect(0, 0, W, H);
		const L = 46,
			R = 14,
		...
		if (done) {
			...
			ctx.setLineDash([5, 3]);
			ctx.beginPath();
			ctx.moveTo(X(0), Yp(P1));
			ctx.lineTo(X(SWITCH), Yp(P1));
			ctx.moveTo(X(SWITCH), Yp(P2));
			ctx.lineTo(X(N - 1), Yp(P2));
			ctx.stroke();
			ctx.setLineDash([]);
			ctx.fillStyle = C.red;
			ctx.font = '10px ' + F;
		...
		if (!done) {
			...
			ctx.lineWidth = 1.4;
			ctx.beginPath();
			ctx.moveTo(L, Yp(PERIODS[curRow]));
			ctx.lineTo(L + pw, Yp(PERIODS[curRow]));
			ctx.stroke();
		}
