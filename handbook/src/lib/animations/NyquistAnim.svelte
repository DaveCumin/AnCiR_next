<script>
	...
	import { C, setupCanvas, drawAxes, drawHGrid, initPalette } from '$lib/animUtils.js';
	...
	const NYQUIST_PCT = (((NYQUIST_LIMIT - MIN_IV) / (MAX_IV - MIN_IV)) * 100).toFixed(2);
	...
	function drawCanvas(canvas, T_s, isTop) {
		...
		if (isTop) {
			...
			ctx.textAlign = 'right';
			ctx.fillText(
				ok ? '✓ Nyquist satisfied (Ts < 12 h)' : '✗ Below Nyquist (Ts \u2265 12 h)',
				padL + pW - 5,
				padT + 13
			);
		} else {
			...
			if (T_alias !== null) {
				...
				ctx.font = 'bold 9.5px Inter,sans-serif';
				ctx.textAlign = 'right';
				ctx.fillText(`Apparent period \u2248 ${T_alias.toFixed(1)} h`, padL + pW - 5, padT + 13);
...
<div class="anim-embed" style={height ? `height: ${height}` : undefined}>
	...
	<div class="controls">
		<button class="btn" bind:this={btnPlay} onclick={nyquistToggle}>&#9654; Play</button>
