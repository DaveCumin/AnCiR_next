<script>
	...
	function drawGroups(k) {
		...
		ctx.font = '11px ' + F;
		ctx.textAlign = 'right';
		ctx.fillText(shuffling ? 'shuffled A' : 'group A', L - 8, T + ph * 0.3 + 4);
		...
		[
			...
			ctx.lineWidth = 2.5;
			ctx.beginPath();
			ctx.moveTo(X(m), row - 14);
			ctx.lineTo(X(m), row + 14);
			ctx.stroke();
		});
		...
		ctx.fillText(
			shuffling
				? `difference for this shuffle = ${d.toFixed(2)}`
				: `observed difference = ${OBS.toFixed(2)}`,
			L + pw / 2,
			T - 10
		);
