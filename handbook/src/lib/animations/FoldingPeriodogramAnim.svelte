<script>
	...
	import { C, setupCanvas, drawAxes, drawHGrid, seededRNG, initPalette } from '$lib/animUtils.js';
	...
	const SWEEP = PERIODS.map((p) => {
		const f = fold(p);
		return { p, Ap: f.Ap, Qp: f.Qp, crit: chiCrit(f.df) };
	});
	...
	/* ── timeline ───────────────────────────────────────────────────────────── */
	const TOTAL = 1000;
	const B = { foldEnd: 0.3, binEnd: 0.56, statEnd: 0.7 };
	const STAGE_START = [0, 0.3, 0.56, 0.7];
	...
	function drawRaw(t) {
		...
		const kShown =
			stage === 0 ? clamp(Math.floor(prog(t, 0, B.foldEnd) * F24.K) + 1, 0, F24.K) : F24.K;

		for (let k = 0; k < F24.K; k++) {
			const x0 = X(k * F24.P),
				x1 = X(Math.min((k + 1) * F24.P, NPT - 1));
	...
	function drawFold(t) {
		...
		const W = cFold.offsetWidth,
			H = cFold.offsetHeight;
		if (!W || !H) return;
		ctx.clearRect(0, 0, W, H);
		const L = 42,
			R = 14,
		...
		const binsShown =
			stage === 0
				? 0
				: stage === 1
					? clamp(Math.floor(prog(t, B.foldEnd, B.binEnd) * P) + 1, 0, P)
					: P;
		...
		if (stage === 1 && binsShown > 0) {
			...
			for (let k = 0; k < K; k++) {
				...
				ctx.beginPath();
				ctx.arc(X(c), Y(SERIES[idx]), 2.6, 0, 2 * Math.PI);
				ctx.fill();
			}
		}
		...
		ctx.setLineDash([5, 3]);
		ctx.beginPath();
		ctx.moveTo(L, Y(GRAND));
		ctx.lineTo(L + pw, Y(GRAND));
		ctx.stroke();
		ctx.setLineDash([]);
		ctx.fillStyle = C.red;
		...
		if (binsShown > 0) {
			...
			for (let c = 0; c < binsShown; c++) {
				ctx.beginPath();
				ctx.arc(X(c), Y(fd.means[c]), 2.2, 0, 2 * Math.PI);
				ctx.fill();
			}
			if (stage === 1) {
				const c = binsShown - 1;
				...
				ctx.lineWidth = 2.4;
				ctx.beginPath();
				ctx.moveTo(X(c), Y(fd.means[c]));
				ctx.lineTo(X(c), Y(GRAND));
				ctx.stroke();
			}
		}
	...
	function drawStat(t) {
		...
		const W = cStat.offsetWidth,
			H = cStat.offsetHeight;
		if (!W || !H) return;
		ctx.clearRect(0, 0, W, H);
		const L = 46,
			R = 40,
		...
		if (stage < 3) {
			...
			ctx.fillStyle = C.navy;
			ctx.beginPath();
			ctx.arc(X(SWEEP[upto].p), Yq(SWEEP[upto].Qp), 4, 0, 2 * Math.PI);
			ctx.fill();
			...
			ctx.rotate(-Math.PI / 2);
			ctx.textAlign = 'center';
			ctx.fillText('Qₚ (χ²)', 0, 0);
			ctx.restore();
			ctx.fillStyle = C.gold;
			ctx.save();
			ctx.translate(W - 12, T + ph / 2);
			ctx.rotate(Math.PI / 2);
			ctx.textAlign = 'center';
			ctx.fillText('Aₚ (amplitude)', 0, 0);
			ctx.restore();
			ctx.strokeStyle = C.border;
			ctx.lineWidth = 1;
			ctx.strokeRect(L, T, pw, ph);
	...
	function updateChrome(t) {
		const stage = stageOf(t);
		if (legendEl) {
			// eslint-disable-next-line svelte/no-dom-manipulating -- imperative animation transport; these label/button nodes are written by the rAF loop, not rendered by Svelte
			legendEl.innerHTML =
				stage < 3
		...
		if (statBox) {
			// eslint-disable-next-line svelte/no-dom-manipulating -- imperative animation transport; these label/button nodes are written by the rAF loop, not rendered by Svelte
			statBox.innerHTML = `<div class="fp-row"><span class="fp-lab">Trial period P</span><span class="fp-val">${period} h → ${fd.P} bins</span></div>
	...
	function draw(f) {
		...
		if (foldTitle)
			// eslint-disable-next-line svelte/no-dom-manipulating -- imperative animation transport; these label/button nodes are written by the rAF loop, not rendered by Svelte
			foldTitle.textContent =
				stageOf(t) < 3
					? 'Folded onto one cycle · bin means emerge'
					: 'Folded waveform · at ' + activeFold(t).period + ' h (the trial period)';
		if (statTitle)
			// eslint-disable-next-line svelte/no-dom-manipulating -- imperative animation transport; these label/button nodes are written by the rAF loop, not rendered by Svelte
			statTitle.textContent =
				stageOf(t) < 3
					? 'Spread of the bin means → the statistic'
					: 'Periodogram — Aₚ and Qₚ vs trial period';
		// eslint-disable-next-line svelte/no-dom-manipulating -- imperative animation transport; these label/button nodes are written by the rAF loop, not rendered by Svelte
...
<div class="anim-embed" style={height ? `height: ${height}` : undefined}>
	<div class="fp-stage-nav">
		{#each ['1 · Fold', '2 · Bin (average)', '3 · Statistic', '4 · Sweep period'] as lbl, i (i)}
			<button class="fp-stage-btn" class:active={activeStage === i} onclick={() => jump(i)}
				>{lbl}</button
			>
		{/each}
	</div>
...
<style>
	...
	.fp-stat-box :global(.fp-row) {
		display: flex;
		justify-content: space-between;
		gap: 10px;
		padding: 2px 0;
	}
	.fp-stat-box :global(.fp-row.fp-sep) {
		border-top: 1px dashed var(--border);
		margin-top: 4px;
		padding-top: 6px;
	}
	.fp-stat-box :global(.fp-lab) {
		color: var(--muted);
	}
	...
	.fp-stat-box :global(.fp-val.gold) {
		color: var(--gold);
	}
	.fp-stat-box :global(.fp-val.navy) {
		color: var(--navy);
	}
