<script>
	...
	let kBH = 0;
	SORTED.forEach((d) => {
		if (d.p <= (d.rank / M) * ALPHA) kBH = d.rank;
	});
	const BONF = ALPHA / M;
	...
	function draw(f) {
		...
		ctx.fillText(
			sortT < 1 ? 'test (original order)' : 'test, ranked by p-value',
			L + pw / 2,
			T + ph + 18
		);
		ctx.save();
