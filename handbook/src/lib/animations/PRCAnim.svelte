<script>
	...
	function draw(p) {
		...
		ctx.fillRect(ctX(0), strip.y, ctX(12) - ctX(0), strip.h);
		ctx.fillStyle = C.blueLt;
		ctx.fillRect(ctX(12), strip.y, ctX(24) - ctX(12), strip.h);
		...
		ctx.font = '9px ' + F;
		ctx.textAlign = 'left';
		ctx.fillText('← earlier (advance)', strip.x + 4, strip.y + strip.h - 5);
		ctx.textAlign = 'right';
		ctx.fillText('later (delay) →', strip.x + strip.w - 4, strip.y + strip.h - 5);
		...
		if (collecting) {
			...
			ctx.lineWidth = 1.4;
			line(ctX(12), strip.y, ctX(12), strip.y + strip.h);
			ctx.setLineDash([]);
			const col = shift >= 0 ? C.green : C.red;
			ctx.strokeStyle = col;
			ctx.lineWidth = 2.4;
			line(ctX(onsetS), strip.y, ctX(onsetS), strip.y + strip.h);
			if (Math.abs(shift) > 0.08) {
				const ay = strip.y + strip.h / 2,
					x0 = ctX(12),
					x1 = ctX(onsetS),
					dir = Math.sign(x1 - x0);
				ctx.strokeStyle = col;
				...
				ctx.beginPath();
				ctx.moveTo(x1, ay);
				ctx.lineTo(x1 - dir * 7, ay - 4);
				ctx.lineTo(x1 - dir * 7, ay + 4);
				ctx.closePath();
				ctx.fill();
			}
			ctx.fillStyle = C.gold;
			ctx.fillRect(ctX(d.ct) - 2, strip.y - 6, 4, strip.h + 12);
		...
		// ── bottom plot ──
		ctx.fillStyle = 'rgba(197,34,31,.08)';
		ctx.fillRect(ctX(12), plot.y, ctX(18) - ctX(12), plot.h);
		ctx.fillStyle = C.greenLt;
		ctx.fillRect(ctX(18), plot.y, ctX(24) - ctX(18), plot.h);
		ctx.fillStyle = 'rgba(0,0,0,.03)';
		ctx.fillRect(ctX(0), plot.y, ctX(12) - ctX(0), plot.h);
		ctx.strokeStyle = C.border;
		ctx.lineWidth = 1;
		ctx.strokeRect(plot.x, plot.y, plot.w, plot.h);
		ctx.setLineDash([3, 3]);
		ctx.strokeStyle = C.axis;
		line(plot.x, dY(0), plot.x + plot.w, dY(0));
		ctx.setLineDash([]);
		ctx.fillStyle = C.muted;
		ctx.font = '10px ' + F;
		ctx.textAlign = 'center';
		for (let t = 0; t <= 24; t += 6) {
			line(ctX(t), plot.y + plot.h, ctX(t), plot.y + plot.h + 4);
			ctx.fillText('CT' + t, ctX(t), plot.y + plot.h + 16);
		}
		...
		ctx.textAlign = 'center';
		ctx.fillText('phase shift ΔΦ (h)   + advance / − delay', 0, 0);
		ctx.restore();
		ctx.font = 'italic 10px ' + F;
		ctx.textAlign = 'center';
		ctx.fillStyle = C.muted;
		ctx.fillText('dead zone', (ctX(0) + ctX(12)) / 2, plot.y + 13);
		ctx.fillStyle = C.red;
		ctx.fillText('delays', (ctX(12) + ctX(18)) / 2, plot.y + 13);
		ctx.fillStyle = C.green;
		ctx.fillText('advances', (ctX(18) + ctX(24)) / 2, plot.y + 13);
		...
		// mean ± SEM
		if (summAlpha > 0) {
			ctx.strokeStyle = hexA(C.text, summAlpha);
			ctx.fillStyle = hexA(C.text, summAlpha);
			ctx.lineWidth = 2;
			DATA.forEach((d) => {
				const x = ctX(d.ct),
					yhi = dY(d.mean + d.sem),
					ylo = dY(d.mean - d.sem);
				line(x, yhi, x, ylo);
				line(x - 4, yhi, x + 4, yhi);
				line(x - 4, ylo, x + 4, ylo);
				ctx.beginPath();
				ctx.arc(x, dY(d.mean), 3.5, 0, 2 * Math.PI);
				ctx.fill();
			});
		}
		...
		if (fitProg > 0) {
			...
			for (let t = 0; t <= end + 0.001; t += 0.15) {
				const x = ctX(t),
					y = dY(prc(t));
		...
		if (stepInd) {
			...
			stepInd.textContent = collecting
				? 'Sampling CT' + DATA[curIdx].ct + '…'
				: summAlpha < 1
					? 'Summarising (mean ± SEM)…'
					: fitProg < 1
						? 'Fitting smooth PRC…'
						: 'PRC complete';
		}
...
<div class="anim-embed" style={height ? `height: ${height}` : undefined}>
	...
	<div class="legend">
		...
		<div class="legend-item">
			<div class="legend-swatch" style="background: var(--color-info)"></div>
			Individual animal (ΔΦ)
		</div>
		<div class="legend-item">
			<div class="legend-swatch" style="background: var(--text)"></div>
			Mean ± SEM
		</div>
		<div class="legend-item">
