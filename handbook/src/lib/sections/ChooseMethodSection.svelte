...
<ChapterSection id="choosing" num="Guide" title="Choosing a Method: The Bigger Picture">
	<p class="chapter-intro">
		...
		<em>question</em>. This guide is the map between the two. Find your goal in the index below and
		it will point you at the right method, the chapter that explains it, and a worked
		<a href="#recipes">recipe</a> to copy. Before that, one distinction shapes almost every choice: whether
		your data come from one individual or from many.
	</p>

	<NoteBox title="Look before you quantify">
		<p>
			Whatever your goal, the first step is always the same: plot an
			<a href="#ch5">actogram</a> and the raw series, and look at them. Every method below returns numbers
			even from noise; the actogram is how you keep those numbers honest.
		</p>
	</NoteBox>

	<h3 class="section-head">One individual over time, or many individuals?</h3>
	<p>
		How you collected the data decides what you can ask of it. Two designs sit behind almost every
		method, and mixing them up is a common source of wrong conclusions (Refinetti, Cornélissen &amp;
		Halberg 2007 <sup class="cite"><a href="#ref-1">[1]</a></sup>).
	</p>
	<div class="design-grid">
		<div class="design-card">
			<h4>Longitudinal — one individual, many cycles</h4>
			<p>
				The same subject measured repeatedly across many days. This is the only design that lets you
				estimate <strong>that individual's own period</strong>
				and watch its phase or amplitude drift over time. Periodograms, correlograms, single-subject cosinor
				and PRCs all assume it. The catch: one individual is a sample of one, so you cannot yet generalise
				to a population.
			</p>
		</div>
		<div class="design-card">
			...
			<p>
				...
				<strong>clock time of every sample recorded</strong>. You cannot recover a within-individual
				period from a single time point, but you can describe the
				<strong>group's</strong> average rhythm by fitting a cosinor at the expected period: a rhythm
				is present if its amplitude differs significantly from zero. You can also compare groups. This
				is the usual design for clinical and field studies.
			</p>
		</div>
	</div>
	<p class="design-takeaway">
		<strong>Rule of thumb:</strong> questions about a rhythm's <em>period</em>, or how it changes
		over time, need a <em>longitudinal</em> record from a single individual. Questions that
		<em>compare groups</em>
		need <em>many</em>
		individuals, with the sampling time of every point. Measuring the
		<em>period</em> is the one thing a cross-sectional design cannot do: folding many individuals onto
		clock time assumes the period rather than revealing it, so an unknown period must come from a longitudinal
		record. The &ldquo;Sampling&rdquo; column below says which design each question wants.
	</p>

	<h3 class="section-head">Start from your goal</h3>
	<p>
		Pick the row that matches the question you actually have. The method column names what to reach
		for; <strong>Sampling</strong> and <strong>Spacing</strong>
		say what the data must look like for that method to be valid, and the last column links to the chapter
		and, where one exists, a ready-made recipe. A
		<em>Spacing</em> of &ldquo;Even&rdquo; means the method reads position in the sequence rather than
		clock time; hover it for what to use instead on gappy data.
	</p>
	...
	<div class="goal-table-wrap">
		<table class="goal-table">
			...
			<tbody>
				{#each GOALS as row (row.id)}
					<tr>
						<td class="goal-cell">
							<!-- eslint-disable-next-line svelte/no-at-html-tags -- goal/method strings are markup literals in src/lib/methodGuide.js, not user input -->
						...
						<td class="go-cell">
							{#each row.go as [label, id], i (id)}<a href="#{id}">{label}</a
								>{#if i < row.go.length - 1}<span class="sep">·</span>{/if}{/each}
						</td>
					</tr>
				{/each}
			</tbody>
	...
	<p>
		The same index, filtered. Tell it your question and what your data look like and it keeps only
		the routes that stay valid, showing the ruled-out ones greyed with the reason, because <em
			>why</em
		> a method is out is usually the part worth knowing.
	</p>
	<MethodPicker />

	<h3 class="section-head">Ordinary statistics: which test?</h3>
	<p>
		The rows above are circadian questions. Underneath them sit ordinary statistical ones, and the
		same goal-first logic applies. Each row names the AnCiR node; <a href="#stats"
			>Statistical Foundations</a
		...
		<table class="goal-table">
			...
			<tbody>
				...
				<tr>
					...
					<td
						>Shapiro-Wilk on the <em>residuals</em>, read alongside a plot, never as an automatic
						gate</td
					>
				...
				<tr>
					...
					<td
						>Pearson if linear; Spearman if skewed, curved-but-monotonic, or outlier-driven. Plot
						first.</td
					>
	...
	<NoteBox title="How this fits with the rest of the handbook">
		<p>
			...
			<a href="#ch13">method table</a> summarises what each method is best suited to. When in doubt, start
