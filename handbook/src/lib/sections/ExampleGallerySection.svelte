<script>
	...
	import { SESSIONS, chapterScores } from '$lib/exampleChapters.js';
	...
	const counts = sessions.reduce((m, s) => ((m[s.kind] = (m[s.kind] ?? 0) + 1), m), {});
	...
	const filtered = $derived.by(() => {
		...
		return sessions.filter((s) => {
			...
			if (!q) return true;
			return (s.keywords ?? `${s.name} ${s.summary} ${s.family}`).toLowerCase().includes(q);
		});
	});
...
<section id="example-gallery">
	...
	<p class="lede">
		Every ready-made session that ships with AnCiR, each one a complete, wired workflow you can open
		on the canvas and take apart. Search by keyword, filter by kind, and click <strong
			>Open in AnCiR</strong
		> to load a session straight from the browser. The chapter chips show where each example is explained.
		This catalogue is bundled with the handbook, so it works offline once this page has loaded.
	</p>
		...
		{#each filtered as s (s.id)}
			<article class="gallery-card">
				...
				{#if s.showcases?.length}
					<div class="gc-nodes">
						<!-- eslint-disable-next-line svelte/require-each-key -- showcase node names are a static positional list and can repeat, so no stable unique key exists -->
				...
				{#if s.chapters?.length}
					<div class="gc-chapters">
						{#each s.chapters as c (c)}
							<a class="gc-chapter" href="#{c}">{c.replace('ch', 'Ch ')}</a>
						{/each}
					</div>
				{/if}
