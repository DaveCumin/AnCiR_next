...
<ChapterSection id="ch7" num="Chapter 7" title="Cosinor Analysis">
  ...
  <p>The <strong>two-argument</strong> arctangent (<code>atan2</code>) is essential — plain <code>arctan</code> cannot separate φ from φ + π and would place the <a class="gloss" href="#gloss-acrophase">acrophase</a> at the trough for half of all datasets. With φ defined this way the model is A·cos(2πt/τ + φ), which peaks at t<sub>peak</sub> = −φ·τ/2π (mod τ). The classical cosinor convention instead reports φ itself, in <strong>negative degrees</strong> from a stated reference time (0°), with 360° = one period; the two differ only in sign and units. <strong>AnCiR reports every acrophase as the time of peak</strong>, in hours after the start of the x column (t = 0), wrapped into [0, τ): the Cosinor node's <code>acrophase</code> port, its panel ("Acrophase (time of peak)"), its stats table, Fit waveform model and Moving analysis all give the same number, and the confidence interval is centred on it. Whichever convention you report, <strong>always state the reference time</strong>, and note that when τ ≠ 24 h the acrophase drifts in clock time across the record.</p>
  ...
  <p>A free period also needs a <strong>search range</strong>. Left unbounded, the optimiser can lower the residual sum of squares by stretching one cosine across a trend or a non-sinusoidal waveform instead of fitting the rhythm; on real activity records this produced "periods" of hundreds to thousands of hours. AnCiR therefore fits the free period inside <strong>Min period</strong> to <strong>Max period</strong> (default 1 to 48 h), starts it from the strongest peaks of a least-squares periodogram inside that range, and shows a warning on the node if the fitted period ends on either limit (the data have no clear rhythm inside the range) or the optimiser does not converge. Treat a period on a limit as "no rhythm found in this range", not as an estimate. Inside the range the fit is the best single cosine, which on a short, noisy record can still be a noise peak; narrow the range around the period you expect when you have one.</p>
    ...
    <ol>
      ...
      <li>Choose the mode:
        <ul>
          ...
          <li><strong>Free period:</strong> leave Fixed Period unticked and set <strong>N cosine curves</strong> = 1 to fit a single sinusoid with optimised period and phase. N &gt; 1 fits multiple overlapping cosines. <strong>Min period</strong> and <strong>Max period</strong> (default 1 to 48 h) bound the search; widen them for ultradian or infradian rhythms.</li>
        </ul>
      </li>
