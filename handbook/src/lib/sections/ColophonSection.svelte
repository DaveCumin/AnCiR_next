...
<ChapterSection id="colophon" num="Colophon" title="Citation, Licence, and Versions">
	<p class="chapter-intro">
		This handbook is archived and citable. It has its own Zenodo record, separate from AnCiR, so a
		citation resolves to a fixed, preserved snapshot of the text rather than to whatever the website
		happens to serve today, and its version reflects changes to the handbook rather than to the
		software.
	</p>
	...
	<p>
		Cite the Zenodo record. The DOI below is a <em>concept</em> DOI: it always resolves to the most recent
		release, and each release also has its own version DOI if you need to pin a specific one. Please cite
		the version you actually used.
	</p>
	...
	<p>
		If your analysis was carried out in AnCiR, cite the software as well. It is a separate record
		with its own DOI and its own version history.
	</p>
	...
	<p>
		The two works carry different licences, because they are different kinds of thing. They share
	...
	<table class="licence-table">
		...
		<tbody>
			<tr>
				...
				<td>
					Use, modify, and redistribute, including commercially, provided you keep the notices and
					state your changes.
				</td>
			</tr>
			<tr>
				...
				<td>
					Copy, adapt, and reuse, including in teaching and in your own papers, as long as you give
					attribution.
				</td>
			</tr>
		</tbody>
	...
	<p class="small-note">
		If you reuse a figure or an animation from this handbook, the handbook citation above is the
	...
	<p class="small-note">
		Source: <a href={REPO} target="_blank" rel="noopener">{REPO}</a>, with the handbook under
		<code>handbook/</code>. The two Zenodo records are linked to each other, so either one leads to
		the other.
	</p>
</ChapterSection>
