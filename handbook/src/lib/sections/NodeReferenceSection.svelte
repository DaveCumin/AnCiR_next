<script>
	...
	const counts = $derived(nodes.reduce((m, n) => ((m[n.kind] = (m[n.kind] ?? 0) + 1), m), {}));
...
<section id="node-reference">
	...
	<p class="lede">
		Every node available in AnCiR, generated directly from the application so it stays in step with
		the software. Search by name or description, filter by kind, and open any node to see its
		inputs, parameters, outputs, the method it implements (with equations and references), and a
		one-click link to a worked example that opens straight in AnCiR.
	</p>
		...
		{#each filtered as n (n.id)}
			<article class="node-card" class:open={openId === n.id}>
				...
				{#if openId === n.id}
					<div class="node-body">
						{#if n.description}
							<!-- eslint-disable-next-line svelte/require-each-key -- paragraphs split out of one string, rebuilt wholesale; position is the identity -->
						...
						<div class="ports">
							<div class="port-col">
								...
								{#if n.inputs?.length}
									<ul>
										<!-- eslint-disable-next-line svelte/require-each-key -- static port list from the generated node manifest; port names can repeat, so position is the identity -->
							...
							<div class="port-col">
								...
								{#if n.params?.length}
									<ul>
										<!-- eslint-disable-next-line svelte/require-each-key -- static parameter list from the generated node manifest; position is the identity -->
										{#each n.params as p}
											<li>
												<code>{p.name}</code
						...
						{#if n.maths?.length}
							<div class="maths">
								<h4>Method</h4>
								<!-- eslint-disable-next-line svelte/require-each-key -- static list of tex strings; position is the identity -->
								{#each n.maths as m}
									<Formula tex={m} />
								{/each}
							</div>
						...
						{#if n.crosslinks?.length}
							<div class="crosslinks">
								<h4>See also</h4>
								<!-- eslint-disable-next-line svelte/require-each-key -- static crosslink list from the generated node manifest; position is the identity -->
						...
						{#if n.references?.length}
							<div class="refs">
								<h4>References</h4>
								<ul>
									<!-- eslint-disable-next-line svelte/require-each-key -- static reference list from the generated node manifest; position is the identity -->
									{#each n.references as r}
										<li>
											{r.cite}{#if r.doi}
												<!-- eslint-disable svelte/no-navigation-without-resolve -- absolute external DOI URL, not a route of this SvelteKit site -->
												<a
													class="doi"
													href={r.doi.startsWith('http') ? r.doi : `https://doi.org/${r.doi}`}
													target="_blank"
													rel="noopener noreferrer"
													>doi:{r.doi.replace(/^https?:\/\/(dx\.)?doi\.org\//, '')}</a
												>{/if}
