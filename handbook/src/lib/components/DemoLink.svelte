<script>
	...
	const sessionUrl = $derived(/^https?:\/\//.test(session) ? session : `${sessionBase}/${session}`);
	const href = $derived(`${base}/?loadFromURL=${encodeURIComponent(sessionUrl)}`);
</script>

<!-- eslint-disable-next-line svelte/no-navigation-without-resolve -- absolute external URL to the deployed AnCiR app, not a route of this SvelteKit site -->
