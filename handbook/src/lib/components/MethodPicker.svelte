<script>
	...
	import { GOALS, SAMPLING_OPTIONS, SPACING_OPTIONS, verdictFor } from '$lib/methodGuide.js';
	...
	const usable = $derived(results.filter((r) => r.verdict.state !== 'blocked').length);
	const touched = $derived(goalId !== 'all' || sampling !== 'unsure' || spacing !== 'unsure');
...
<div class="picker">
	<div class="picker-head">
		<h4>Method picker</h4>
		<p>
			Answer what you can. Anything left as <em>not sure</em> simply doesn't filter, so you can start
			with only the part you know.
		</p>
	</div>
	...
	<div class="controls">
		<label>
			...
			<select bind:value={goalId}>
				...
				{#each GOALS as g (g.id)}
					<option value={g.id}
						>{g.goal.replace(/<[^>]+>/g, '')}{g.context ? ` (${g.context})` : ''}</option
					>
				{/each}
			</select>
		</label>
		...
		<label>
			...
			<select bind:value={sampling}>
				{#each SAMPLING_OPTIONS as o (o.value)}
		...
		<label>
			...
			<select bind:value={spacing}>
				{#each SPACING_OPTIONS as o (o.value)}
					<option value={o.value}>{o.label}</option>
				{/each}
			</select>
		</label>
	...
	<div class="summary" aria-live="polite">
		...
		{:else if usable === 0}
			<span class="none"
				>No method here answers that question from that kind of data. The reasons are below.</span
			>
		{:else}
			<span
			...
			<li class="result {r.verdict.state}">
				<div class="result-head">
					...
					<span class="q">
						...
						{@html r.goal}{#if r.context}<span class="q-context">{r.context}</span>{/if}
					</span>
				</div>
				...
				{#if r.verdict.state !== 'blocked'}
					<p class="meta">
						...
						<span class="go">
							{#each r.go as [label, id], i (id)}<a href="#{id}">{label}</a
								>{#if i < r.go.length - 1}<span class="sep">·</span>{/if}{/each}
						</span>
					</p>
				{/if}
			</li>
