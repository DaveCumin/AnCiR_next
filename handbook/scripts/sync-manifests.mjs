...
if (!existsSync(srcNodes) || !existsSync(srcSessions)) {
	warn(
		`AnCiR manifest not found under ${ANCIR}/static — is the handbook still inside the AnCiR workspace?`
	);
	process.exit(1);
}
...
const sessions = (Array.isArray(idx) ? idx : (idx.sessions ?? idx.demos ?? [])).filter(
	(s) => s.kind !== 'dataset'
);
const ref = readJSON(resolve(HANDBOOK_LIB, 'nodeReference.json'));

log(
	`${c.cyan}AnCiR:${c.rst}    ${ANCIR}  (nodes ${nodes.generatedFromVersion}, ${nodes.nodes.length} nodes, ${sessions.length} sessions)`
);
...
if (missingRef.length) {
	problems++;
	warn(
		`${missingRef.length} palette node(s) have NO nodeReference.json entry — add curated prose for:`
	);
	log(`    ${missingRef.join(', ')}`);
}
if (orphanRef.length) {
...
log(
	`${c.grn}✓ nodeReference.json covers every palette node; no orphans; all showcases resolve.${c.rst}`
);
