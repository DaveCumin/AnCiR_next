...
export const PARAM_NOTES = {
	...
	ChiSquared:
		'`testType` is "independence" (two categorical columns: `xIN` = rows, `yIN` = columns → contingency table → test of association), "goodness" (one column `xIN`: a categorical column is tabulated into category counts, a numeric column is read as observed counts, tested against a uniform expectation), or "fisher" (Fisher\'s EXACT test on a 2×2 table from `xIN` × `yIN`). `correction` toggles Yates\' continuity correction, applied only to 2×2 independence tables. `alternative` ("two-sided" / "less" / "greater") is read only in fisher mode. Outputs statistic, pvalue and df. Expected counts below 5 make the χ² approximation unreliable — that is exactly when to switch `testType` to "fisher", which is exact at any sample size. Fisher mode reports pvalue (and an odds ratio in-node) but leaves statistic and df as NaN, because an exact test has neither.',
	CrossCorrelation:
