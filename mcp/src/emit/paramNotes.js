...
export const PARAM_NOTES = {
	...
	ColumnFunctions:
		'Row-wise combination of the `xsIN` columns into one output. `func`: "add" (element-wise sum; concatenates with a space when a column is categorical), "average" (row mean), "min", "max", "sd" (SAMPLE standard deviation across the columns, n-1) or "percentile" (type-7/linear-interpolation quantile across the columns; `percentile` 0-100, default 50 = the row median; missing/blank cells are dropped per row). Only "percentile" reads `percentile`.',
	MovingAnalysis:
		'One analysis in a sliding window. `analysis` (decides which params matter): "summary", "periodogram", "cosinor", "npcra", "fft", "correlogram", "rectfit", "doublelogistic" or "trend". "summary" emits rolling descriptive stats per window — mean, sd (SAMPLE standard deviation, n-1) and percentile (type-7/linear-interpolation quantile; `summaryPercentile` 0-100, 50 = median); blank/missing cells are dropped, not zeros. "periodogram" reads `pgMethod` ("Lomb-Scargle"/"Chi-squared"/"Enright") + `periodMin`/`periodMax`/`periodStep` (HOURS); "trend" reads `trendModel` ("linear"/"exponential"/"logarithmic"/"polynomial", last uses `trendPolyDegree`); "cosinor" reads `useFixedPeriod`/`fixedPeriod`/`nHarmonics` (also emits `rel_amplitude`); "npcra" reads `npcraEpochHours`/`npcraPeriod`/`npcraMWindow`/`npcraLWindow`. `windowSize`/`stepSize` are in x-axis units. `binLabel` ("start"/"center"/"end") is where the window\'s result is stamped.',
	SmoothedData:
	...
	FitFunction:
		'`model` is "cosinor", "rectangular" or "doublelogistic". For a periodic fit keep `useFixedPeriod:true`, `fixedPeriod` in HOURS; a free-period cosinor (`useFixedPeriod:false`, `Ncurves`) stays inside `minPeriod`..`maxPeriod` (HOURS, default 1..48). `permuteTest:true` enables the permutation test (then `nPermutations`/`permutationSeed`/`permutationStatistic` = "rSquared" or "rmse" apply). `fix*`/`fixed*` pairs pin a parameter (e.g. `fixKappa:true` → `fixedKappa`).',
	Cosinor:
		'For a daily rhythm keep `useFixedPeriod:true`, `fixedPeriod` in HOURS (e.g. 24). `nHarmonics` (fixed-period) adds harmonics; `Ncurves` (free-period) is the number of components; the free period is searched for and kept inside `minPeriod`..`maxPeriod` (HOURS, default 1..48) and the node warns if it ends on a bound. Every acrophase (port, panel, CSV) is the time of peak in hours after the start. `referenceHrs` (HOURS) is the phase-angle reference. `alpha` (0.05). `permuteTest:true` enables the permutation test (`nPermutations`/`permutationSeed`/`permutationStatistic`).',
	DoubleLogistic:
