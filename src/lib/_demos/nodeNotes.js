...
export const NODE_NOTES = {
	...
	RayleighTest:
		"Circular-statistics tests for phase/angle columns (radians, degrees, or clock-hours on a chosen period), with a test selector. The default Rayleigh test asks each column whether its phases (e.g. acrophases, activity-onset times) point in a preferred direction rather than scattering uniformly, reporting the mean resultant length R (0 = dispersed, 1 = tightly clustered), Rayleigh's z = nR², and a p-value (Batschelet 1981). Switching the test to Watson-Williams runs the circular analogue of one-way ANOVA across the selected columns (each a group of angles), reporting an F statistic and p-value for whether the groups share a common mean direction; it assumes reasonably concentrated, similarly-dispersed samples and applies a von Mises concentration correction (Watson & Williams 1956).",
	RectangularWave:
