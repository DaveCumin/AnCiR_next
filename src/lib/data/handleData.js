...
import { componentMap } from '$lib/components/ProcessStep.svelte';
// @ts-ignore
...
export function createnewDataForGraph(protoValues, protoOther) {
	...
	for (let i = 0; i < get(data).length; i++) {
		contextMenu.update((menu) => {
			...
			menu.funcs.push(() => addDataToGraph(get(data)[i].id, protoValues, protoOther));
			return menu;
		});
	}
}
...
export function addDataToGraph(tableID_IN, prototypechartvalues, prototypeother) {
	...
	graphs.update((current) => {
		...
		Object.keys(current[get(activeGraphTab)].sourceData[L]).forEach((k) => {
			if (Object.keys(current[get(activeGraphTab)].sourceData[L][k]).includes('hex')) {
				current[get(activeGraphTab)].sourceData[L][k].hex = getRandomHexColour();
			}
		});
...
export function removeNaNs(arraysIN) {
	...
	if (!Array.isArray(arraysIN) || arraysIN.length === 0 || !arraysIN.every(Array.isArray)) {
...
export function addManualMarker(time) {
	...
	if (sem?.text == 'addingManualMarker') {
		graphs.update((currentData) => {
			currentData[get(activeGraphTab)].chartData.onsets[sem.sourceIndex][sem.o].onsetTimes.push(
				time
			);
...
export function removeMarker(srcIndex, osIndex, d) {
	graphs.update((currentData) => {
		currentData[get(activeGraphTab)].chartData.onsets[srcIndex][osIndex].onsetTimes.splice(d, 1);
		return currentData;
	});
}
