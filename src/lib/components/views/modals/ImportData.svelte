<script module>
	...
	import { guessDateofArray, normalizeTimeFormat } from '$lib/utils/time/TimeUtils';
	...
	export async function loadFromURL(url) {
		try {
			...
			// A session built by our AI Worker has no author-chosen layout, so tidy it. Only
			// for OUR host: a user's own session URL may carry a layout they arranged
			// deliberately, and re-tidying it would throw their work away.
			if (isTrustedSessionUrl(url)) {
				// WorkflowEditor watches this counter and re-runs the layered layout once the
				// nodes have rendered and been measured (it debounces, which matters here:
				// plots finish asynchronously after their workers return).
				appState.tidyLayoutRequest = (appState.tidyLayoutRequest ?? 0) + 1;
				addNotification(
					'This session was generated by AI. Check the analyses and parameters before relying on the results.',
					'warning',
					12000
				);
			}
