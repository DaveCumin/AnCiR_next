<script>
	...
	let { showDropdown = $bindable(false), dropdownTop = 0, dropdownLeft = 0,
...
<Dropdown bind:showDropdown top={dropdownTop} left={dropdownLeft}>
	{#snippet groups()}
		<div class="dropdown-action">
			<button onclick={() => addNewColumn(tableId)}> Add New Column </button>
		</div>

		<div class="dropdown-action">
			<button onclick={() => makeNewTablePlot(tableId)}> View as Table </button>
		</div>
	{/snippet}
</Dropdown>