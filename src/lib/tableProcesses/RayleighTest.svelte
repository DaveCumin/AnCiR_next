<script module>
	...
	// @ts-nocheck
	// Rayleigh test of circular uniformity for phase/angle columns.
	// Each Y column is a sample of angles; the node reports the mean resultant
	// length R, Rayleigh's z, and the p-value (test of a preferred direction) as
	// scalar-metric ports — one value per Y in yIN order, like Cosinor/NPCRA.
	import { getColumnById } from '$lib/core/Column.svelte';
	import { rayleighTest, circularMean, toRadians } from '$lib/utils/circular.js';
	...
	const defaults = new Map([
		...
		// Scalar-metric output ports (one value per y input, in yIN order).
		['out', { R: { val: -1 }, z: { val: -1 }, pvalue: { val: -1 } }],
		['valid', { val: false }],
	...
	// Pure evaluation: returns { perY: { [yId]: {n,R,z,pValue,meanAngle} }, anyValid }.
	...
	function writeRayleighMetrics(argsIN, result) {
		...
		for (const key of METRIC_KEYS) {
			const field = key === 'pvalue' ? 'pValue' : key;
			const arr = yINs.map((yId) => perY[yId]?.[field] ?? NaN);
...
<script>
	...
	let rayleighData = $state({ perY: {}, anyValid: false, yINs: [] });
	...
	// Metric out-columns already exist for every session because the keys are
	// fixed (R/z/pvalue). Backfill + reconcile the fixed set.
	$effect(() => {
		p.args.yIN;
		if (!mounted) return;
		queueMicrotask(() =>
