...
describe('demo-workflow-free-running actogram: legacy marker blocks', () => {
	it('a legacy onset block still fits both parameters over the real data', () => {
		...
		const reg = m.linearRegression;
		// The fit runs on the UNWRAPPED onsets (onsetUnwrap.js). Each row's template
		// search spans that row and the next, and here every row's detection is the
		// NEXT row's onset, with rows 3 and 4 finding the same one. The unwrap places
		// each at the time it really fell and merges the duplicate, leaving 13 distinct
		// onsets. The demo is generated with tau = 24.8 h; the per-row reading used
		// before (misassigned rows, the duplicate counted twice) gave 24.749, this
		// gives 24.808. Pinned to the last bit: the unconstrained path is the same
		// compensated arithmetic as the least-squares helper.
		expect(m.onsetUnwrap.n).toBe(13);
		expect(m.onsetUnwrap.nMerged).toBe(1);
		expect(reg.slope).toBe(24.807692307692307);
		expect(reg.intercept).toBe(-0.3076923076923077);
		expect(reg.rSquared).toBe(0.9999914154247648);
		expect(reg.rmse).toBe(0.27196414661021184);
		expect(m.estimatedPhase).toEqual({ phase: 0.5, refDay: 1 });
		// A clean free-running record raises no reason to distrust tau.
		expect(m.onsetWarnings).toEqual([]);
	});
