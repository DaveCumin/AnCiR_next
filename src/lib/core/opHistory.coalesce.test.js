...
function addPlot(name = 'p') {
	applyOp({
		kind: 'addPlot',
		plotData: { type: 'scatterplot', name, x: 0, y: 0, width: 1, height: 1 }
	});
...
describe('different targets do not coalesce', () => {
	...
	it('different setProcessArg targets (same key) do not coalesce', () => {
		const colA =
			(applyOp({ kind: 'addColumn', columnData: { name: 'A', type: 'number' } }), core.data[0].id);
		const colB =
			(applyOp({ kind: 'addColumn', columnData: { name: 'B', type: 'number' } }), core.data[1].id);
		applyOp({
			kind: 'addProcess',
			columnId: colA,
