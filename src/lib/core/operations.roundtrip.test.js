...
describe('round-trip: apply op then its inverse restores prior state', () => {
	it('addPlot → removePlot leaves no plots', () => {
		roundTrip({
			kind: 'addPlot',
			plotData: { type: 'scatterplot', x: 1, y: 2, width: 3, height: 4 }
		});
	...
	it('removePlot → addPlot restores the plot with the same id and props', () => {
		applyOp({
			kind: 'addPlot',
			plotData: { type: 'scatterplot', name: 'p', x: 5, y: 6, width: 7, height: 8 }
		});
	...
	it('setPlotProperty round-trips back to the original value', () => {
		applyOp({
			kind: 'addPlot',
			plotData: { type: 'scatterplot', name: 'orig', x: 0, y: 0, width: 1, height: 1 }
		});
	...
	it('setPlotPosition round-trips x/y/width/height', () => {
		applyOp({
			kind: 'addPlot',
			plotData: { type: 'scatterplot', x: 10, y: 11, width: 12, height: 13 }
		});
		const id = core.plots[0].id;
	...
	it('removeProcess → addProcess restores the process at the same index with same id and args', () => {
		...
		applyOp({
			...
			columnId: colId,
			processType: 'Normalize',
			args: { normalizationType: 'a' }
		});
		applyOp({
			kind: 'addProcess',
			columnId: colId,
			processType: 'Normalize',
			args: { normalizationType: 'b' }
		});
...
describe('no-op / nonexistent-target branches return null', () => {
	...
	it('setPlotProperty with unchanged value returns null', () => {
		applyOp({
			kind: 'addPlot',
			plotData: { type: 'scatterplot', name: 'same', x: 0, y: 0, width: 1, height: 1 }
		});
		const id = core.plots[0].id;
	...
	it('setProcessArg on a missing column returns null', () => {
		expect(
			applyOp({ kind: 'setProcessArg', columnId: 999, processId: 1, key: 'k', value: 1 })
		).toBeNull();
	});
	...
	it('setProcessArg on a missing process returns null', () => {
		...
		const colId = core.data[0].id;
		expect(
			applyOp({ kind: 'setProcessArg', columnId: colId, processId: 99999, key: 'k', value: 1 })
		).toBeNull();
	});
...
describe('double-remove and idempotency', () => {
	...
	it('removing a plot twice: second remove is a null no-op', () => {
		applyOp({
			kind: 'addPlot',
			plotData: { type: 'scatterplot', x: 0, y: 0, width: 1, height: 1 }
		});
		const id = core.plots[0].id;
